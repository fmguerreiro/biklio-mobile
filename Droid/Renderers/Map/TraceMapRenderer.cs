...
namespace Trace.Droid {
	...
	public class TraceMapRenderer : MapRenderer, GoogleMap.IInfoWindowAdapter, IOnMapReadyCallback {
		...
		public void OnMapReady(GoogleMap googleMap) {
			...
			EventHandler<GoogleMap.MyLocationChangeEventArgs> a = null;
			a = (object sender, GoogleMap.MyLocationChangeEventArgs e) => {
				var cu = CameraUpdateFactory.NewLatLngZoom(new LatLng(e.Location.Latitude, e.Location.Longitude), 16.0f);
				map.MoveCamera(cu);
				map.MyLocationChange -= a;
			};
			map.MyLocationChange += a;
